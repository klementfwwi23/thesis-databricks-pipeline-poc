...
Builds star-schema style dimensions and facts plus a simple aggregate mart
(top items per month) on top of the silver layer for analytics consumption.
"""
...
# Target Unity Catalog location for all gold tables written by this job.
CATALOG = "workspace"
SCHEMA = "imperative"
...
def nullif_zero(column):
    """
    Convert zero values to null to avoid divide-by-zero in ratio calculations.

    Used in percentage metrics like margin or discount rates.
    """
    return F.when(column == 0, F.lit(None)).otherwise(column)


def build_gold_dim_date() -> DataFrame:
    """Create a business-friendly date dimension with calendar and fiscal attributes."""
    return spark.table(fq_table("silver_date_dim")).select(
        ...
        "year_month",
        # Human-readable "Qn YYYY" representation.
        F.concat(
            F.lit("Q"),
            F.col("d_qoy").cast("string"),
            F.lit(" "),
            F.col("d_year").cast("string"),
        ).alias("quarter_year"),
        F.when(F.col("d_holiday") == "Y", True)
        .otherwise(False)
        .alias("is_holiday"),
        "is_weekend",
        F.when(F.col("d_following_holiday") == "Y", True)
        .otherwise(False)
        .alias("is_following_holiday"),
        F.when(F.col("d_current_day") == "Y", True)
        .otherwise(False)
        .alias("is_current_day"),
        F.when(F.col("d_current_week") == "Y", True)
        .otherwise(False)
        .alias("is_current_week"),
        F.when(F.col("d_current_month") == "Y", True)
        .otherwise(False)
        .alias("is_current_month"),
        F.when(F.col("d_current_quarter") == "Y", True)
        .otherwise(False)
        .alias("is_current_quarter"),
        F.when(F.col("d_current_year") == "Y", True)
        .otherwise(False)
        .alias("is_current_year"),
        # Self-referencing keys for period-over-period analysis.
...
def build_gold_dim_item() -> DataFrame:
    """Create an item dimension with price bands and margin percentage."""
    return spark.table(fq_table("silver_item")).select(
        ...
        # Simple price banding to support analyses by price segment.
        ...
        # Margin percentage expressed relative to current retail price.
        F.round(
            (F.col("i_current_price") - F.col("i_wholesale_cost"))
            / nullif_zero(F.col("i_current_price"))
            * 100,
            2,
        ).alias("margin_pct"),
        ...
        # Current = record is still open-ended.
        F.when(F.col("i_rec_end_date").isNull(), True)
        .otherwise(False)
        .alias("is_current"),
        F.col("ingestion_timestamp").alias("source_timestamp"),
    )
...
def build_gold_dim_store() -> DataFrame:
    """Create a store dimension with store size category and a composed full address."""
    ...
    return source.select(
        ...
        # Size category for store footprint analyses.
        ...
        # Street-level and fully formatted address variants for reporting.
        F.concat_ws(" ", "s_street_number", "s_street_name", "s_street_type").alias(
            "street_address"
        ),
        ...
        # A store is considered currently open if the record is still valid and no closed date exists.
        F.when(
            F.col("s_rec_end_date").isNull() & F.col("s_closed_date_sk").isNull(), True
        )
        .otherwise(False)
        .alias("is_currently_open"),
        F.col("ingestion_timestamp").alias("source_timestamp"),
    )


def build_gold_dim_customer() -> DataFrame:
    """Create a customer dimension with age bands and preference flags."""
    ...
    return spark.table(fq_table("silver_customer")).select(
        ...
        F.concat_ws(" ", "c_salutation", "c_first_name", "c_last_name").alias(
            "full_name"
        ),
        ...
        # Simple generational banding used for segmentation examples.
        ...
        .alias("age_group"),
        F.when(F.col("c_birth_year").isNull(), F.lit(None))
        .otherwise(age)
        .alias("current_age"),
        F.when(F.col("c_preferred_cust_flag") == "Y", True)
        .otherwise(False)
        .alias("is_preferred_customer"),
...
def build_gold_fact_store_sales() -> DataFrame:
    """
    Create the store_sales fact table with derived discount and margin metrics.

    This table is the main transaction fact in the star schema and is optimized
    for BI consumption (one row per sold line item).
    """
    discount_ratio = (F.col("ss_list_price") - F.col("ss_sales_price")) / nullif_zero(
        F.col("ss_list_price")
    )
    return spark.table(fq_table("silver_store_sales")).select(
        ...
        # Discount-related metrics and bands for promotional analysis.
        F.round(discount_ratio * 100, 2).alias("discount_percentage"),
        F.round(
            F.col("ss_net_profit") / nullif_zero(F.col("ss_ext_sales_price")) * 100, 2
        ).alias("profit_margin_percentage"),
        F.round(
            F.col("ss_ext_sales_price") / nullif_zero(F.col("ss_quantity")), 2
...
def build_gold_top_items_month() -> DataFrame:
    """
    Build an aggregate mart with the top-selling items per year-month.

    This is a narrow, consumption-ready gold table for common reporting use
    cases like revenue leaderboards.
    """
    ...
    # Join fact with date and item dimensions to expose business attributes.
    joined_sales = (
    ...
    # Aggregate to item-month level for ranking.
    item_month_sales = (
        joined_sales.groupBy(
            "year",
            "month",
            "year_month",
            "item_key",
    ...
    # Rank items by revenue (then quantity) within each month and keep top N.
    window = Window.partitionBy("year", "month").orderBy(
        F.col("total_revenue").desc(), F.col("total_quantity_sold").desc()
    )
    return (
...
def main() -> None:
    """Build and persist all gold tables defined in this module."""
